...
const textsEasy = [
  ...
  { text: "Д@е#т$и %и*г(р)а[ю]т {в} ф/у|т`б~о`л", answer: "Текст про детей, которые играют в футбол", options: ["Текст про детей, которые играют в футбол", "Текст про детей, которые играют в хоккей", "Текст про детей, которые играют в шахматы"] },
  ...
  { text: "У@ч#и%т^е&л*ь (о)б[ъ]я{с}н/я|е`т у~р`о`к", answer: "Текст про учителя, который объясняет урок", options: ["Текст про учителя, который объясняет урок", "Текст про врача, который лечит больных", "Текст про строителя, который строит дом"] },
  { text: "К@н#и%г^а &л*е(ж)и[т] {н}а /с|т`о~л`е", answer: "Текст про книгу, которая лежит на столе", options: ["Текст про книгу, которая лежит на столе", "Текст про тетрадь, которая лежит в портфеле", "Текст про журнал, который лежит на полке"] },
  { text: "Р@ы#б%а ^п*л(ы)в[а]е{т} /в| `р~е`к`е", answer: "Текст про рыбу, которая плавает в реке", options: ["Текст про рыбу, которая плавает в реке", "Текст про утку, которая плывёт по озеру", "Текст про дельфина, который живёт в море"] },
  ...
  { text: "М@о#р%о^ж&е*н(о)е [ч]а{с}т/о |т`а~е`т", answer: "Текст про мороженое, которое часто тает", options: ["Текст про мороженое, которое часто тает", "Текст про пирожное, которое очень вкусное", "Текст про конфету, которую любят дети"] },
  { text: "В@е#л%о^с&и*п(е)д [с]т{о}и/т |в` г~а`р`а`ж`е", answer: "Текст про велосипед, который стоит в гараже", options: ["Текст про велосипед, который стоит в гараже", "Текст про машину, которая едет по дороге", "Текст про мотоцикл, который мчится быстро"] },
  { text: "С@н#е%г ^и*д(е)т [б]е{л}ы/й |и` ~м`я`г`к`и`й", answer: "Текст про снег, который идёт белый и мягкий", options: ["Текст про снег, который идёт белый и мягкий", "Текст про дождь, который льёт как из ведра", "Текст про туман, который окутал город"] },
  ...
  { text: "Д@е#р%е^в&о *з(е)л[е]н{о} /и |в`ы~с`о`к`о`е", answer: "Текст про дерево, которое зелёное и высокое", options: ["Текст про дерево, которое зелёное и высокое", "Текст про куст, который низкий и густой", "Текст про траву, которая мягкая и зелёная"] },
  ...
  { text: "Ч@а#й%к^а &п*ь(ё)т [ч]а{й} /н|а` ~к`у`х`н`е", answer: "Текст про чайку, которая пьёт чай на кухне", options: ["Текст про чайку, которая пьёт чай на кухне", "Текст про ворону, которая сидит на заборе", "Текст про голубя, который клюёт крошки"] },
  { text: "Л@и#с%а ^р*ы(б)а[ч]и{т} /н|а `~р`е`к`е", answer: "Текст про лису, которая рыбачит на реке", options: ["Текст про лису, которая рыбачит на реке", "Текст про медведя, который ловит рыбу", "Текст про рыбака, который сидит с удочкой"] }
];
// ===== СРЕДНИЙ УРОВЕНЬ =====
const textsMedium = [
  { text: "С@е#г$о%д^н&я *б(ы)л[о] {п}р/е|к`р~а`с`н`о. М@ы #с $%х^о&д*и(л)и [в] {п}а/р|к. Т`а~м м@ы #в $%с^т&р*е(т)и[л]и {д}р/у|з`е~й.", answer: "Текст про поход в парк и встречу с друзьями", options: ["Текст про поход в парк и встречу с друзьями", "Текст про учёбу в школе и домашнее задание", "Текст про игру дома и просмотр мультфильмов", "Текст про поездку к бабушке в деревню"] },
  { text: "В@ч#е$р%а ^м&ы *с(м)о[т]р{е}л/и |ф`и~л`ь`м. Ф@и #л$ь%м ^б&ы*л (о)ч[е]н{ь} /и|нт~е`р`е`с`н`ы`м.", answer: "Текст про просмотр интересного фильма", options: ["Текст про просмотр интересного фильма", "Текст про чтение увлекательной книги", "Текст про посещение театра", "Текст про игру в компьютерную игру"] },
  { text: "К@о#т $с%и^д&е*л (н)а [п]о{д}о/к|о`н~н`и`к`е. О@н #с %м^о&т*р(е)л [н]а {п}т/и|ц. П@т #и%ц ^ы *б(ы)с[т]р{о} /у|л`е~т`е`л`и.", answer: "Текст про кота на подоконнике, который смотрит на птиц", options: ["Текст про кота на подоконнике, который смотрит на птиц", "Текст про кота, который спит на диване", "Текст про кота, который играет с клубком", "Текст про кота, который охотится на мышей"] },
  { text: "М@а#л$ь%ч^и&к *и(г)р[а]е{т} /н|а с~к`р`и`п`к`е. И о@н #у %м^е&е*т (х)о[р]о{ш}о /п|ры~г`а`т`ь. Д@р #у%з^ь&я *п(о)д[д]е{р}ж/и|в`а~ю`т.", answer: "Текст про мальчика, который играет на скрипке", options: ["Текст про мальчика, который играет на скрипке", "Текст про мальчика, который играет в футбол", "Текст про мальчика, который рисует пейзаж", "Текст про мальчика, который поёт в хоре"] },
  { text: "Д@е#в%о^ч&к*а (л)ю[б]и{ла} /р|ис~о`в`а`т`ь. О@н #а &р*и(с)о[в]а{л} /а|кв~а`р`е`л`ью`. П@о #л%у^ч&а*л(о)с[ь] {о}ч/е|н`ь ~к`р`а`с`и`в`о.", answer: "Текст про девочку, которая любит рисовать акварелью", options: ["Текст про девочку, которая любит рисовать акварелью", "Текст про девочку, которая любит лепить из пластилина", "Текст про девочку, которая любит вышивать крестиком", "Текст про девочку, которая любит собирать конструктор"] },
  { text: "Н@а #с%т^о&л*е (л)е[ж]и{т} /я|бл~о`к`о. О@н #о %к^р&у*г(л)о[е] {и} /к|ра~с`н`о`е. Я @х #о%ч ^у&е*г [о] /с|ъе~с`т`ь.", answer: "Текст про красное яблоко на столе", options: ["Текст про красное яблоко на столе", "Текст про зелёное яблоко на столе", "Текст про грушу на столе", "Текст про апельсин на столе"] },
  { text: "М@ы #с $м%о ^т&р*и(м) [т]е{л}е/в|и`з~о`р. П@о #к %а^з&ы*в(а)ю[т] {м}у/л|ьт~и`к`и. Э@т #о %^о&ч*е(н)ь [и]н{т}е/р|е`с~н`о.", answer: "Текст про просмотр интересных мультфильмов", options: ["Текст про просмотр интересных мультфильмов", "Текст про просмотр новостей по телевизору", "Текст про просмотр спортивного матча", "Текст про просмотр кулинарного шоу"] },
  { text: "М@а #м%а ^п&е*к(л)а [п]и{р}о/г|. В@с #е %м ^б&ы*л(о) [о]ч{е}н/ь| в~к`у`с`н`о. М@ы # %е *л(и) [е]г{о} /в|с`е~й с`е`м`ь`ё`й.", answer: "Текст про маму, которая испекла вкусный пирог", options: ["Текст про маму, которая испекла вкусный пирог", "Текст про маму, которая сварила суп", "Текст про маму, которая пожарила котлеты", "Текст про маму, которая купила торт"] },
  { text: "Д@е #т %и ^п&о*ш(л)и [в] {ш}к/о|л`у. У@ч #и %т ^е&л*ь(н)и[ц]а {с}к/а|за~л`а (пр)и[в]ет. Д@е #т %и ^б &у*д(у)т [у]ч{и}т/ь|с`я.", answer: "Текст про детей, которые пошли в школу", options: ["Текст про детей, которые пошли в школу", "Текст про детей, которые пошли в магазин", "Текст про детей, которые пошли в кино", "Текст про детей, которые пошли на прогулку"] },
  { text: "Н@а #у %л^и&ц*е (в)е[с]н{а}. Т@а #е %т ^с&н*е(г). [П]о{ч}т/и| р`а`с`т`а`я`л. В@с #е %р ^а&д*у(ю)т[с]я {т}е/п|л`у.", answer: "Текст про весну, когда тает снег и становится тепло", options: ["Текст про весну, когда тает снег и становится тепло", "Текст про зиму, когда мороз и снегопад", "Текст про осень, когда листопад и дожди", "Текст про лето, когда жара и купание"] },
  { text: "В@о #д %в ^о&р*е (б)о[л]ь{ш}о/й| п~р`у`д. Т@а #м %ы ^б &у*д(е)м [к]у{п}а/т|ьс~я. Е@щ #е %т ^а &м* (е)с[т]ь {б}е/с|е`д~к`а.", answer: "Текст про большой пруд и купание", options: ["Текст про большой пруд и купание", "Текст про глубокое озеро и рыбалку", "Текст про широкую реку и катание на лодке", "Текст про тёплое море и пляж"] },
  { text: "П@а #п %а ^к &у*п(и)л [м]а{ш}и/н|у. Н@о #в %а ^я &м*а(ш)и[н]а {к}р/а|с`н`а`я. О@н #а % ^б &ы*с(т)р[а]я {и} /б|л`е~с`т`я`щ`а`я.", answer: "Текст про папу, который купил красную машину", options: ["Текст про папу, который купил красную машину", "Текст про папу, который купил синюю машину", "Текст про папу, который купил новый велосипед", "Текст про папу, который купил мотоцикл"] },
  { text: "М@е #д %в ^е &д*ь (л)ю[б]и{т} /м|яс~о. О@н #ж %и ^в &ё*т (в) [л]е{с}у. З@и #м %о ^й & *о(н) [с]п{и}т/| в б~е`р`л`о`г`е.", answer: "Текст про медведя, который живёт в лесу и спит зимой", options: ["Текст про медведя, который живёт в лесу и спит зимой", "Текст про волка, который живёт в стае и воет ночью", "Текст про лису, которая живёт в норе и охотится", "Текст про зайца, который живёт в поле и быстро бегает"] },
  { text: "У@ч #и %т ^е &л*ь (п)р[о]в{е}р/я|ет. Д@е #т %и ^п &и*ш(у)т [в] {т}е/т|р`а~д`я`х. У@р #о %к ^б &ы*л (о)ч[е]н/ь| и~н`т`е`р`е`с`н`ы`м.", answer: "Текст про урок, где дети пишут в тетрадях", options: ["Текст про урок, где дети пишут в тетрадях", "Текст про урок физкультуры на стадионе", "Текст про урок музыки в классе", "Текст про урок рисования в мастерской"] },
  { text: "Б@а #б %у ^ш &к*а (и)с[п]е{к}л/а| п~и`р`о`г. П@и #р %о ^г &п*о(л)у[ч]и{л}с/я| с~л`а`д`к`и`м. М@ы #в %с ^е & *е(г)о [с]ъ{е}л/и.", answer: "Текст про бабушку, которая испекла сладкий пирог", options: ["Текст про бабушку, которая испекла сладкий пирог", "Текст про маму, которая испекла пирожки", "Текст про тётю, которая купила торт", "Текст про сестру, которая сделала кекс"] },
  { text: "В@и #т %я ^р &и*с(у)е[т] {п}е/й|за~ж. Н@а #к %а ^р &т*и(н)е [о]з{е}р/о. П@о #л %у ^ч &и*л(о)с[ь] {к}р/а|с`и~в`о.", answer: "Текст про Витю, который рисует пейзаж с озером", options: ["Текст про Витю, который рисует пейзаж с озером", "Текст про Машу, которая рисует портрет", "Текст про Петю, который рисует натюрморт", "Текст про Олю, которая рисует городской пейзаж"] },
  { text: "З@и #м %а ^п &р*и(ш)л[а] {с}м/о|р`о~з`о`м. Н@а #р %е ^к &е* (л)ё[д] {т}о/л|с`т~ы`й. Д@е #т %и ^к &а*т(а)ю[т]с[я] {с}г/о|рк~и.", answer: "Текст про зиму, когда дети катаются на санках с горки", options: ["Текст про зиму, когда дети катаются на санках с горки", "Текст про зиму, когда дети играют в снежки", "Текст про зиму, когда дети лепят снеговика", "Текст про зиму, когда дети катаются на коньках"] },
  { text: "М@ы #п %о ^е &д*е(м) [в] {п}у/т|е`ш~е`с`т`в`и`е. Т@у #д %а ^ & *в(с)ё [б]у{д}е/т| н~о`в`ы`м. К@а #ж %д ^ы &й *д(е)н[ь] {р}а/д|у`е~т.", answer: "Текст про путешествие, которое приносит радость", options: ["Текст про путешествие, которое приносит радость", "Текст про поход, который укрепляет здоровье", "Текст про экскурсию, которая даёт знания", "Текст про прогулку, которая поднимает настроение"] }
];
// ===== СЛОЖНЫЙ УРОВЕНЬ =====
const textsHard = [
  { text: "У@ч#ё$н%ы^е &о*б(н)а[р]у{ж}и/л|и н`о~в`о`е о`т`к`р`ы`т`и`е. О@н #и %д ^о &к*а(з)а[л]и {ч}т/о| з`н`а`ч`е`н`и`е в`е`л`и`к`о. Э@т #о %п ^о &м*о(ж)е[т] {п}о/в|л`и~я`т`ь н`а м`и`р. Н@о #с %е ^й &ч*а(с) [с]о{з}д/а|ю`т~с`я н`о`в`ы`е т`е`х`н`о`л`о`г`и`и.", answer: "Текст про новое научное открытие, которое может изменить мир", options: ["Текст про новое научное открытие, которое может изменить мир", "Текст про историческое событие, которое повлияло на ход истории", "Текст про космическое явление, которое удивило учёных", "Текст про биологическое исследование, которое спасло жизни", "Текст про физический эксперимент, который доказал теорию", "Текст про химическую реакцию, которая произошла случайно"] },
  { text: "Д@р#е%в^н&я *б(ы)л[а] {м}а/л|е`н~ь`к`о`й. Т@а #м %ж ^и &л*и (л)и[ш]ь {д}в/е| с`е`м`ь`и. С@е #й %ч ^а &с* (т)а[м] {п}о/с|т`р~о`и`л`и н`о`в`ы`е д`о`м`а. Н@о #в %с ^е & *р(а)в[н]о {б}ы/л|о т`и~х`о. М@е #с %т ^н &ы*е (ж)и[т]е[л]и /б|ы`л~и д`о`в`о`л`ь`н`ы.", answer: "Текст про маленькую деревню, где недавно построили новые дома", options: ["Текст про маленькую деревню, где недавно построили новые дома", "Текст про большой город со старыми историческими зданиями", "Текст про среднее село с современными магазинами и школой", "Текст про мегаполис с высотными небоскрёбами", "Текст про посёлок с деревянными резными избами", "Текст про хутор с фермерскими хозяйствами"] },
  { text: "Э@к#с%п^е&д*и(ц)и[я] {о}т/п|р`а~в`и`л`а`с`ь в п`у`т`ь. К@о #м %а ^н &д*а (н)а[ш]л{а} /к|л`а~д. Н@а #й %д ^к &а *б(ы)л[а] {з}а/п|р`я~т`а`н`а д`а`в`н`о. О@н #и %с ^к &а*л(и) [е]г{о} /д|о`л~г`о. В@с #ё %з ^а &к*о(н)ч[и]л{о}с/ь| у`с~п`е`х`о`м.", answer: "Текст про экспедицию, которая отправилась на поиски древнего клада", options: ["Текст про экспедицию, которая отправилась на поиски древнего клада", "Текст про путешественников, которые покоряли высокие горы", "Текст про учёных, которые изучали глубины океана", "Текст про строителей, которые возводили огромный мост", "Текст про космонавтов, которые летали в открытый космос", "Текст про археологов, которые раскопали древний город"] },
  { text: "И@с#к%у^с&с*т(в)е[н]н{ы}й /и|н`т~е`л`л`е`к`т р`а`з`в`и`в`а`е`т`с`я. П@о #м %о ^г &а*е(т) [л]ю{д}я/м| в р`а~б`о`т`е. Н@о #е %с ^т &ь *и (р)и[с]к{и}. Б@у #д %у ^щ &е*е (з)а[в]и{с}и/т| о~т т`о`г`о к`а`к е`г`о и`с`п`о`л`ь`з`у`ю`т.", answer: "Текст про искусственный интеллект, который помогает людям, но несёт риски", options: ["Текст про искусственный интеллект, который помогает людям, но несёт риски", "Текст про робототехнику, которая автоматизирует производство", "Текст про компьютерные программы, которые ускоряют работу", "Текст про кибербезопасность, которая защищает данные", "Текст про цифровые технологии, которые меняют жизнь", "Текст про виртуальную реальность, которая открывает новые миры"] },
  { text: "И@с#с%л^е&д*о(в)а[н]и{я} /п|о`к~а`з`а`л`и в`а`ж`н`ы`е р`е`з`у`л`ь`т`а`т`ы. У@ч #ё %н ^ы &е* (с)д[е]л{а}л/и| в`ы~в`о`д`ы. Э@т #о %п ^о &м*о(ж)е[т] [и]з{м}е/н|и`т~ь б`у`д`у`щ`е`е. В@с #ё %т ^е &п*е(р)ь [п]о{д}р/у|г`о~м в`ы`г`л`я`д`и`т.", answer: "Текст про важное исследование, которое поможет в будущем", options: ["Текст про важное исследование, которое поможет в будущем", "Текст про научный прорыв, который изменил представления", "Текст про технологическую революцию, которая ускорила прогресс", "Текст про медицинское открытие, которое спасло миллионы", "Текст про экологический проект, который очистил природу", "Текст про социальный эксперимент, который показал новое"] },
  { text: "П@у#т%е^ш&е*с(т)в[и]е {б}ы/л|о д`л~и`т`е`л`ь`н`ы`м. М@ы #п %о ^с &е*т(и)л[и] {м}н/о|г`о~ с`т`р`а`н. В@и #д %е ^л &и* (н)о[в]ы{е} |г`о~р`о`д`а. Э@т #о %б ^ы &л*о (н)е[з]а{б}ы/в|а`е~м`о. М@ы #в %е ^р &н*у(л)и[с]ь {д}о/м|о`й~ с ч`у`д`е`с`н`ы`м`и в`о`с`п`о`м`и`н`а`н`и`я`м`и.", answer: "Текст про долгое путешествие, где мы посетили много новых стран и городов", options: ["Текст про долгое путешествие, где мы посетили много новых стран и городов", "Текст про короткую поездку, которая запомнилась надолго", "Текст про опасное приключение, которое закончилось благополучно", "Текст про семейный отпуск, который прошёл весело", "Текст про образовательную экскурсию, которая дала много знаний", "Текст про командный поход, который сплотил друзей"] },
  { text: "В@е#л%и^к&а*я (б)и[б]л{и}о/т|е`к~а б`ы`л`а о`г`р`о`м`н`о`й. Т@а #м %х ^р &а*н(и)т[с]я {м}н/о|г`о~ к`н`и`г. К@а #ж %д ^а &я *к(н)и[г]а {р}а/с|с`к~а`з`ы`в`а`е`т с`в`о`ю и`с`т`о`р`и`ю. Ч@и #т %а ^т &е*л(и) [п]р{и}х/о|д`я~т ч`и`т`а`т`ь и у`з`н`а`в`а`т`ь н`о`в`о`е.", answer: "Текст про большую библиотеку, где хранится много удивительных книг", options: ["Текст про большую библиотеку, где хранится много удивительных книг", "Текст про старинный архив, где лежат древние документы", "Текст про книжный магазин, где продаются новинки", "Текст про читальный зал, где тихо и уютно", "Текст про музей книги, где выставлены редкие экземпляры", "Текст про издательство, где печатают новые книги"] },
  { text: "С@п#о%р^т &с*е(г)о[д]н{я} /п|р`и~н`ё`с п`о`б`е`д`у. Н@а #ш %а ^к &о*м(а)н[д]а {и}г/р|а`л~а о`т`л`и`ч`н`о. Ф@и #н %а ^л &ь*н(ы)й [с]ч{ё}т/ ч`ё~т`к`о п`о`к`а`з`а`л р`е`з`у`л`ь`т`а`т. В@с #е %р ^а &д*ы(и) [п]р{а}з/д|н`у~ю`т п`о`б`е`д`у.", answer: "Текст про спортивный матч, где наша команда одержала победу", options: ["Текст про спортивный матч, где наша команда одержала победу", "Текст про футбольный чемпионат, где боролись лучшие клубы", "Текст про олимпийские игры, где выступали сильнейшие спортсмены", "Текст про тренировку, которая прошла очень продуктивно", "Текст про соревнования по плаванию, где установили рекорд", "Текст про шахматный турнир, где победил юный гений"] },
  { text: "М@у#з%ы^к&а *з(в)у[ч]а{л}а /н|а в`с~ю с`ц`е`н`у. О@р #к %е ^с &т*р(а) [и]г{р}а/л| д`о~л`г`о. Л@ю #д %и ^а &п*л(о)д[и]р{о}в/а|л`и~ с в`о`с`т`о`р`г`о`м. В@е #ч %е ^р & *б(ы)л [п]р{е}к/р|а`с~н и н`е`з`а`б`ы`в`а`е`м.", answer: "Текст про музыкальный концерт, где играл большой оркестр", options: ["Текст про музыкальный концерт, где играл большой оркестр", "Текст про оперный театр, где выступали знаменитые певцы", "Текст про джазовый фестиваль, где собрались лучшие музыканты", "Текст про рок-концерт, где было много зрителей", "Текст про классическую музыку, которую любят во всём мире", "Текст про талантливого композитора, который создал шедевры"] },
  { text: "П@о#ж%а^р &б*ы(с)т[р]о {л}и/к|в`и~д`и`р`о`в`а`л`и. П@о #ж %а ^р &н*ы(е) [п]р{и}б/ы|л`и~ с`р`а`з`у. В@с #е %х ^ &ж*и(т)е[л]и {б}л/а|г`о~д`а`р`и`л`и и`х. Д@о #м % ^а & *б(ы)л [с]п{а}с/ё|н`н~ы`м и н`е п`о`с`т`р`а`д`а`л.", answer: "Текст про пожар, который быстро потушили смелые пожарные", options: ["Текст про пожар, который быстро потушили смелые пожарные", "Текст про наводнение, от которого спасали жителей", "Текст про землетрясение, где помогали волонтёры", "Текст про ураган, который разрушил всё на своём пути", "Текст про аварию, которую предотвратили в последний момент", "Текст про спасение людей, которые оказались в беде"] },
  { text: "К@о#с%м^о&н*а(в)т [в]ы{ш}е/л| в о`т~к`р`ы`т`ы`й к`о`с`м`о`с. О@н #п %р ^о &в*е(л) [н]а{с}т/р|о`й~к`у о`б`о`р`у`д`о`в`а`н`и`я. З@а #з %в ^е &з*д(ё)з[д]ы {в}с/е|х п`о~р`а`ж`а`л`и. Т@е #п %е ^р &ь* (м)о[ж]н{о} /л|е`т~е`т`ь д`а`л`ь`ш`е.", answer: "Текст про космонавта, который вышел в открытый космос", options: ["Текст про космонавта, который вышел в открытый космос", "Текст про астронома, который открыл новую звезду", "Текст про ракету, которая отправилась к Марсу", "Текст про спутник, который вращается вокруг Земли", "Текст про телескоп, который видит далёкие галактики", "Текст про луноход, который исследует поверхность Луны"] },
  { text: "И@с#т%о^р&и*я (д)р[е]в{н}я/я| и и`н~т`е`р`е`с`н`а. В@е #д %у ^т &р*а(с)к[о]п{к}и /у|ж`е~ м`н`о`г`о л`е`т. А@р #х %е ^о &л*о(г)и [н]а{ш}л/и| р`е~д`к`и`е н`а`х`о`д`к`и. Н@о #в %ы ^е & *а(р)т[е]ф{а}к/т|ы и`з~у`ч`а`ю`т с`е`й`ч`а`с.", answer: "Текст про древнюю историю и археологические раскопки", options: ["Текст про древнюю историю и археологические раскопки", "Текст про средневековые замки и рыцарей", "Текст про древний Рим и гладиаторов", "Текст про Египет и строительство пирамид", "Текст про Грецию и олимпийских богов", "Текст про Китай и Великую стену"] },
  { text: "Э@к#о%л^о&г*и(я) [в]а{ж}н/а| д`л~я в`с`е`х. Н@а #м %у ^ж &н*о (б)е[р]е{ч}ь /п|р`и~р`о`д`у. Ч@е #л %о ^в &е*к (д)о[л]ж{е}н /п|о`м~н`и`т`ь о б`у`д`у`щ`е`м. В@с #ё %з ^а &в*и(с)и[т] {о}т/ н`а~с.", answer: "Текст про экологию и защиту природы", options: ["Текст про экологию и защиту природы", "Текст про загрязнение океана пластиком", "Текст про вымирающих животных и их спасение", "Текст про леса и проблему вырубки деревьев", "Текст про воздух и важность чистоты атмосферы", "Текст про воду и необходимость её экономии"] },
  { text: "М@е#д%и^ц&и*н(а) [с]п{а}с/а|е`т~ ж`и`з`н`и. В@р #а %ч ^и & *у(ч)а[т]с[я] {в}с/ю| ж`и~з`н`ь. Н@о #в %ы ^е & *л(е)к[а]р{с}т/в|а х`о~р`о`ш`о п`о`м`о`г`а`ю`т. В@с #ё %э ^т &о* д(е)л[а]е{т} /м|и`р~ л`у`ч`ш`е.", answer: "Текст про медицину, которая спасает жизни", options: ["Текст про медицину, которая спасает жизни", "Текст про врачей, которые лечат людей", "Текст про лекарства, которые помогают выздороветь", "Текст про больницы, где лечат сложные болезни", "Текст про здоровье и важность профилактики", "Текст про здоровый образ жизни и спорт"] },
  { text: "К@у#л%и^н&а*р(и)я [э]т{о} /и|с`к~у`с`с`т`в`о. Ш@е #ф %п ^о &в*а(р) [д]о{л}ж/е|н т`а~л`а`н`т`л`и`в`ы`м. К@а #ж %д ^о &е* (б)л[ю]д{о} /п|р`о~д`у`м`а`н`о д`о м`е`л`о`ч`е`й. В@с #ё %э ^т &о* р(а)д[у]е{т} /г|о`с~т`е`й.", answer: "Текст про кулинарию и искусство приготовления еды", options: ["Текст про кулинарию и искусство приготовления еды", "Текст про рестораны и известных шеф-поваров", "Текст про национальные блюда разных стран", "Текст про полезную и здоровую пищу", "Текст про сладости и десерты", "Текст про традиции застолья"] },
  { text: "С@п#о%р^т &з*а(к)а[л]я{е}т /х|а`р~а`к`т`е`р. О@н #у %ч ^и &т* (д)и[с]ц{и}п/л|и`н~е. П@о #б %е ^д &ы* (п)р[и]н{о}с/я|т р`а~д`о`с`т`ь. В@с #ё %э ^т &о* д(е)л[а]е{т} /с|и`л~ь`н`е`е.", answer: "Текст про спорт, который закаляет характер и учит дисциплине", options: ["Текст про спорт, который закаляет характер и учит дисциплине", "Текст про победы и достижения спортсменов", "Текст про командный дух и взаимовыручку", "Текст про здоровый образ жизни и активность", "Текст про тренировки и преодоление себя", "Текст про соревнования и честную борьбу"] },
  { text: "Т@е#х%н^о&л*о(г)и[и] {р}а/з|в`и~в`а`ю`т`с`я б`ы`с`т`р`о. К@а #ж %д ^ы &й *д(е)н[ь] {н}о/в|о`е~ п`о`я`в`л`я`е`т`с`я. М@ы #е %д ^е &м* (в) [п]е{р}е/д| н`е~ о`с`т`а`н`а`в`л`и`в`а`я`с`ь. Б@у #д %у ^щ &е*е б`у~д`е`т е`щ`ё б`о`л`е`е т`е`х`н`о`л`о`г`и`ч`н`ы`м.", answer: "Текст про технологии, которые развиваются с каждым днём", options: ["Текст про технологии, которые развиваются с каждым днём", "Текст про смартфоны и компьютеры", "Текст про интернет и социальные сети", "Текст про искусственный интеллект и роботов", "Текст про будущее и научную фантастику", "Текст про новости техники и гаджеты"] },
  { text: "О@б#р%а^з&о*в(а)н[и]е {д}а/ё|т з`н~а`н`и`я. О@н #о %^о &т*к(р)ы[в]а{е}т /д|в`е~р`и к в`о`з`м`о`ж`н`о`с`т`я`м. У@ч #и %т ^ь &с*я (в)с[ю] {ж}и/з|н`ь~ н`е`о`б`х`о`д`и`м`о. В@с #ё %э ^т &о* п(о)м[о]г{а}е/т| р`а~з`в`и`в`а`т`ь`с`я.", answer: "Текст про образование, которое открывает двери к знаниям", options: ["Текст про образование, которое открывает двери к знаниям", "Текст про школу и университет", "Текст про учителей и наставников", "Текст про учёбу и экзамены", "Текст про книги и библиотеки", "Текст про саморазвитие и новые навыки"] },
  { text: "Д@р#у%ж^б&а *п(о)м[о]г{а}е/т| в т`р~у`д`н`ы`е м`о`м`е`н`т`ы. В@м #е %с ^т &е* (п)р[о]б{л}е/м|ы л`е~г`ч`е р`е`ш`а`т`ь. О@н #а % ^д &е*л(а)е[т] {ж}и/з|н`ь~ я`р`ч`е. В@с #ё %э ^т &о* д(а)р[и]т {р}а/д|о`с~т`ь.", answer: "Текст про дружбу, которая помогает в трудные времена", options: ["Текст про дружбу, которая помогает в трудные времена", "Текст про верных друзей и поддержку", "Текст про детство и школьных товарищей", "Текст про совместные увлечения и хобби", "Текст про доверие и взаимопонимание", "Текст про радость общения и встречи"] },
  { text: "И@с#к%у^с&с*т(в)о [в]д{о}х/н|о`в~л`я`е`т. К@а #р %т ^и &н*ы (к)а[р]т{и}н/ы| в`ы~з`ы`в`а`ю`т э`м`о`ц`и`и. М@у #з %ы ^к &а* (д)а[ё]т {э}м/о|ц`и~и и н`а`с`т`р`о`е`н`и`е. В@с #ё %э ^т &о* д(е)л[а]е{т} /ж|и`з~н`ь к`р`а`с`и`в`е`е.", answer: "Текст про искусство, которое вдохновляет и дарит эмоции", options: ["Текст про искусство, которое вдохновляет и дарит эмоции", "Текст про живопись и великих художников", "Текст про музыку и известных композиторов", "Текст про театр и актёрское мастерство", "Текст про литературу и любимых писателей", "Текст про архитектуру и красивые здания"] }
];
...
function renderSplitWords() {
  ...
  const textsMedium = [
    ...
    { text: "пятачокшёлвгостиквиннипухуоннёсвподарокгоршочекмёда", correct: "пятачок шёл в гости к винни пуху он нёс в подарок горшочек мёда", words: ["пятачок", "шёл", "в", "гости", "к", "вини", "пух", "он", "нёс", "подарок", "горшочек", "мёда"] },
...
const textsHard = [
  {
    ...
    correct: "однажды в тёмном лесу жил маленький ёжик каждое утро он выходил искать ягоды и грибы однажды он встретил зайца который предложил ёжику дружбу",
    words: ["однажды","в","тёмном","лесу","жил","маленький","ёжик","каждое","утро","он","выходил","искать","ягоды","и","грибы","однажды","он","встретил","зайца","который","предложил","ёжику","дружбу"]
  },
  {
    ...
    correct: "маша пошла в первый класс учительница встретила её с улыбкой в классе было много новых друзей маша научилась читать писать и очень полюбила учиться",
    words: ["маша","пошла","в","первый","класс","учительница","встретила","её","с","улыбкой","в","классе","было","много","новых","друзей","маша","научилась","читать","писать","и","очень","полюбила","учиться"]
  },
  {
    text: "александрмечталстатьврачомсдетстваонхорошоучилсявшколепоступилвмедицинскийуниверситетнабюджетмногочиталиипрактиковалсявбольницепослеучёбыустроилсявдетскуюполиклинику",
    correct: "александр мечтал стать врачом с детства он хорошо учился в школе поступил в медицинский университет на бюджет много читал и практиковался в больнице после учёбы устроился в детскую поликлинику",
    words: ["александр","мечтал","стать","врачом","с","детства","он","хорошо","учился","в","школе","поступил","в","медицинский","университет","на","бюджет","много","читал","и","практиковался","в","больнице","после","учёбы","устроился","в","детскую","поликлинику"]
  },
  {
    text: "командапрограммистовсоздавалановуюкомпьютернуюигруработанаднейшлатригодадизайнерырисовалиперсонажейипрограммистыписаликодиисправлялиошибкиигрувыпустилинавсехплатформахгеймерыповсемумируоценилиеёвысоко",
    correct: "команда программистов создавала новую компьютерную игру работа над ней шла три года дизайнеры рисовали персонажей и программисты писали код и исправляли ошибки игру выпустили на всех платформах геймеры по всему миру оценили её высоко",
    words: ["команда","программистов","создавала","новую","компьютерную","игру","работа","над","ней","шла","три","года","дизайнеры","рисовали","персонажей","и","программисты","писали","код","и","исправляли","ошибки","игру","выпустили","на","всех","платформах","геймеры","по","всему","миру","оценили","её","высоко"]
  },
  {
    ...
    correct: "в далёкой галактике жил маленький робот спарки он мечтал научиться танцевать но только программы для работы на заводе однажды он встретил девочку лизу которая показала ему радость движения теперь он самый весёлый робот",
    words: ["в","далёкой","галактике","жил","маленький","робот","спарки","он","мечтал","научиться","танцевать","но","только","программы","для","работы","на","заводе","однажды","он","встретил","девочку","лизу","которая","показала","ему","радость","движения","теперь","он","самый","весёлый","робот"]
  }
];
  while (textsHard.length < 20) {
    textsHard.push({
      text: "путешественникиотправилисьвокругосветноепутешествиенаяхтеонипосетилимногостранпознакомилисьсразнымикультурамиивкуснойкухнейкаждаястранаудивлялаихчемтоновымивоспоминанияосталисьнавсюжизнь", 
      correct: "путешественники отправились в кругосветное путешествие на яхте они посетили много стран познакомились с разными культурами и вкусной кухней каждая страна удивляла их чем то новым воспоминания остались на всю жизнь",
...
const matchesMedium = [
  ...
  { starts: ["Зима", "Весна", "Лето", "Осень"], ends: ["холод", "таяние", "жара", "листопад"] },
  { starts: ["Марс", "Венера", "Юпитер", "Сатурн"], ends: ["красная планета", "яркая планета", "самая большая", "кольца"] },
  { starts: ["Понедельник", "Среда", "Пятница", "Воскресенье"], ends: ["начало недели", "середина недели", "конец работы", "выходной"] },
  ...
  { starts: ["Компьютер", "Телефон", "Телевизор", "Планшет"], ends: ["работа", "звонки", "просмотр", "игры"] },
  { starts: ["Учитель", "Врач", "Строитель", "Повар"], ends: ["учит", "лечит", "строит", "готовит"] },
  { starts: ["Море", "Река", "Озеро", "Океан"], ends: ["солёная вода", "пресная вода", "стоячая вода", "самый большой"] },
  { starts: ["Глаза", "Уши", "Нос", "Рот"], ends: ["видят", "слышат", "чуют", "едят"] },
  { starts: ["Самолёт", "Поезд", "Корабль", "Автомобиль"], ends: ["летает", "рельсы", "плывёт", "дорога"] },
  { starts: ["Музыка", "Живопись", "Танец", "Театр"], ends: ["слушают", "смотрят", "смотрят", "смотрят"] },
  { starts: ["Утро", "День", "Вечер", "Ночь"], ends: ["рассвет", "солнце высоко", "закат", "темнота"] },
  { starts: ["Берёза", "Дуб", "Сосна", "Клён"], ends: ["белая кора", "жёлуди", "иголки", "лист"] },
  { starts: ["Воробей", "Голубь", "Ворона", "Синица"], ends: ["маленький", "серый", "чёрная", "жёлтая грудка"] },
  { starts: ["Футбол", "Баскетбол", "Волейбол", "Хоккей"], ends: ["ноги", "руки", "через сетку", "клюшка"] },
  { starts: ["Красный", "Синий", "Жёлтый", "Зелёный"], ends: ["светофор", "небо", "солнце", "трава"] },
  ...
  { starts: ["Улыбка", "Смех", "Плач", "Крик"], ends: ["радость", "веселье", "горе", "громкий звук"] },
  { starts: ["Пингвин", "Страус", "Орёл", "Соловей"], ends: ["не летает", "не летает", "летает", "поёт"] },
  { starts: ["Снег", "Дождь", "Ветер", "Град"], ends: ["белый", "мокрый", "движение воздуха", "ледяной"] },
  { starts: ["Колобок", "Репка", "Курочка Ряба", "Теремок"], ends: ["убежал", "вытянули", "яичко", "живут звери"] },
...
const matchesHard = [
  ...
  { starts: ["Александр Македонский", "Юлий Цезарь", "Наполеон Бонапарт", "Пётр Первый", "Чингисхан"], ends: ["полководец", "правитель", "император", "царь", "хан"] },
  ...
  { starts: ["Роман", "Повесть", "Рассказ", "Поэма", "Стихотворение"], ends: ["большой жанр", "средний жанр", "малый жанр", "поэтический жанр", "стихи"] },
  { starts: ["Комедия", "Трагедия", "Драма", "Мюзикл", "Опера"], ends: ["смешной жанр", "печальный жанр", "серьёзный жанр", "музыкальный жанр", "пение"] },
  { starts: ["Палитра", "Кисть", "Мольберт", "Краски", "Холст"], ends: ["смешивать краски", "рисовать", "держит холст", "цвета", "основа картины"] },
  { starts: ["Карандаш", "Уголь", "Пастель", "Сангина", "Соус"], ends: ["графит", "уголь", "мел", "красный материал", "мягкий материал"] },
  { starts: ["Квартет", "Квинтет", "Секстет", "Септет", "Октет"], ends: ["4", "5", "6", "7", "8"] },
  { starts: ["Саванна", "Джунгли", "Пустыня", "Тундра", "Тайга"], ends: ["травы", "густые леса", "песок", "мох", "хвойный лес"] },
  { starts: ["Меркурий", "Венера", "Земля", "Марс", "Юпитер"], ends: ["ближайшая к солнцу", "яркая планета", "жизнь", "красная планета", "самая большая"] },
  { starts: ["Гомер", "Данте", "Шекспир", "Гёте", "Пушкин"], ends: ["Илиада", "Божественная комедия", "Гамлет", "Фауст", "Евгений Онегин"] },
  { starts: ["Титаник", "Лузитания", "Британик", "Олимпик", "Квин Мэри"], ends: ["1912", "1915", "1916", "1911", "1936"] },
  ...
  { starts: ["Шахматы", "Шашки", "Го", "Рэндзю", "Нарды"], ends: ["логика", "логика", "стратегия", "стратегия", "настольная игра"] },
  { starts: ["До", "Ре", "Ми", "Фа", "Соль"], ends: ["C", "D", "E", "F", "G"] },
  { starts: ["Амфора", "Гидрия", "Кратер", "Килик", "Лекиф"], ends: ["вино", "вода", "смешивание", "чаша", "масло"] },
  { starts: ["Сфинкс", "Пегас", "Цербер", "Химера", "Горгона"], ends: ["лев и человек", "крылатый конь", "трёхглавый пёс", "огнедышащее существо", "змеи вместо волос"] },
  ...
  { starts: ["Атлант", "Прометей", "Эпиметей", "Менетий", "Климен"], ends: ["держит небо", "дал огонь людям", "брат Прометея", "гордость", "титан"] }
];
...
function shuffleWord(word) {
  ...
  } while (shuffled.join('') === word); // чтобы не совпадало с оригиналом
  
  return shuffled.join('');
}

// 🔹 Создание массива анаграмм из слов
...
function renderLogicAnagram() {
  ...
  const wordsEasy = [
    ...
    { word: "дом", hint: "Где ты живёшь?" },
    { word: "сон", hint: "Что мы видим ночью?" },
    { word: "рот", hint: "Чем мы едим?" },
    { word: "нос", hint: "Чем мы нюхаем?" },
    { word: "лес", hint: "Где много деревьев?" },
    { word: "сад", hint: "Где растут яблони?" },
    { word: "парк", hint: "Место для прогулок" },
    { word: "мир", hint: "Когда нет войны" },
    { word: "друг", hint: "Тот, кто поможет" },
    { word: "снег", hint: "Белый зимой" },
    { word: "море", hint: "Много воды" },
    { word: "река", hint: "Течёт вода" },
    { word: "гора", hint: "Высокое место" },
    { word: "поле", hint: "Много травы" },
    { word: "мак", hint: "Красный цветок" },
    { word: "арбуз", hint: "Крупная ягода" },
    { word: "слон", hint: "Большое животное" },
    { word: "банан", hint: "Жёлтый фрукт" },
    { word: "молот", hint: "Инструмент" },
    { word: "собака", hint: "Верный друг человека" },
    { word: "кошка", hint: "Любит молоко" },
  ...
  const wordsMedium = [
    ...
    { word: "собака", hint: "Охраняет дом" },
    { word: "мышка", hint: "Боятся слоны" },
    { word: "птица", hint: "Летает" },
    { word: "волк", hint: "Серый хищник" },
    { word: "лиса", hint: "Рыжая плутовка" },
    { word: "заяц", hint: "Быстро бегает" },
    { word: "белка", hint: "Прыгает по деревьям" },
    { word: "ёжик", hint: "Колючий зверёк" },
    { word: "пчела", hint: "Делает мёд" },
    { word: "ослик", hint: "Длинные уши" },
    { word: "ёлка", hint: "Новогоднее дерево" },
    { word: "сосна", hint: "Хвойное дерево" },
    { word: "берёза", hint: "Белый ствол" },
    { word: "дуб", hint: "Могучее дерево" },
    { word: "клён", hint: "Красивые листья" },
    { word: "осина", hint: "Дрожат листья" },
    { word: "рябина", hint: "Красные ягоды" },
    { word: "липа", hint: "Сердцевидные листья" },
    { word: "тополь", hint: "Даёт пух" }
  ];

  const wordsHard = [
    { word: "учитель", hint: "Даёт знания" },
    { word: "ученик", hint: "Учится" },
    { word: "портфель", hint: "Сумка для школы" },
    { word: "карандаш", hint: "Рисует" },
    { word: "линейка", hint: "Измеряет длину" },
    { word: "тетрадь", hint: "Пишут" },
    { word: "школа", hint: "Где учатся" },
    { word: "перемена", hint: "Отдых" },
    { word: "праздник", hint: "Весёлый день" },
    { word: "каникулы", hint: "Отдых от школы" },
    { word: "завтрак", hint: "Утренний приём пищи" },
    { word: "обед", hint: "Дневной приём пищи" },
    { word: "ужин", hint: "Вечерний приём пищи" },
    { word: "компот", hint: "Напиток" },
    { word: "печенье", hint: "Сладость" },
    { word: "конфета", hint: "Сладкое" },
    { word: "шоколад", hint: "Какао" },
    { word: "мороженое", hint: "Холодный десерт" },
    { word: "кофе", hint: "Бодрит" },
    { word: "жираф", hint: "Самый высокий" },
    { word: "крокодил", hint: "Зелёный хищник" }
  ];

  // 🔹 Генерация
  const anagramsEasy = generateAnagrams(wordsEasy);
  const anagramsMedium = generateAnagrams(wordsMedium);
  const anagramsHard = generateAnagrams(wordsHard);
  ...
  whatMissingTimeouts.push(setTimeout(() => {
    ...
    whatMissingTimeouts.push(setTimeout(() => {
      ...
      whatMissingTimeouts.push(setTimeout(() => {
        ...
        // Берём все картинки для вариантов ответа (исключая те, что уже на экране)
        const visibleFiles = new Set();
        remainingCards.forEach(card => {
          visibleFiles.add(card.dataset.value);
        });
        
        const otherImages = allImages.filter(img => !visibleFiles.has(img.file) && img.file !== missingCard.file);
        const shuffledOthers = [...otherImages].sort(() => Math.random() - 0.5);
        
        // Собираем варианты: правильный + 3 случайных неправильных из других картинок
        let optionItems = [missingCard];
        for (let i = 0; i < 3 && i < shuffledOthers.length; i++) {
          optionItems.push(shuffledOthers[i]);
        }
        ...
        optionItems.forEach(cardItem => {
          const btn = document.createElement("button");
          btn.className = "option-btn-horizontal";
          btn.innerHTML = `<img src="images/${cardItem.file}" style="width: 50px; height: 50px; object-fit: contain; vertical-align: middle;">`;
...
function renderReaction() {
  ...
  // Фоны для разных уровней сложности
  const backgrounds = {
    1: 'images/фон_средний.png',
    2: 'images/фон_сложный.png',
    3: 'images/фон_простой.png'
  };
  ...
  gameArea.innerHTML = `
    ...
    <div class="reaction-field" id="reactionField" style="background-image: url('${backgrounds[state.level]}'); background-size: cover; background-position: center; position: relative;">
...
function renderSchulte() {
  ...
  gameArea.innerHTML = `
    ...
    <div style="display: flex; justify-content: center; margin: 20px 0;">
      <div class="schulte-grid-wrapper" style="background: linear-gradient(135deg, #d6cff5, #d6cff5); padding: 20px; border-radius: 32px; box-shadow: 6px 6px 12px rgba(0,0,0,0.05), -3px -3px 10px rgba(255,255,255,0.8);">
  ...
  style.textContent = `
    ...
    @keyframes schulteCelebrate {
      0% { transform: scale(1); }
      50% { transform: scale(1.05); background: #b7eeca; border-color: #b7eeca; }
